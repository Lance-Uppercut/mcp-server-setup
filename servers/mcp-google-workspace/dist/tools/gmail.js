...
import fs from 'fs';
function decodeBase64Data(fileData) {
...
export class GmailTools {
    ...
    decodeBase64UrlString(base64UrlString) {
        try {
    ...
    getTools() {
        ...
        ].filter(tool => ((process.env.GMAIL_ALLOW_SENDING === 'true')
            ? true
    ...
    async bulkGetEmails(args) {
        ...
        try {
            const emails = await Promise.all(emailIds.map(async (emailId) => {
                const email = await this.gmail.users.messages.get({
                    userId,
                    id: emailId,
                    format: 'full'
    ...
    async reply(args) {
        ...
        try {
            ...
            const headers = originalMessage.data.payload?.headers?.reduce((acc, header) => {
                if (header.name && header.value) {
            ...
            const message = {
                raw: Buffer.from(`In-Reply-To: ${originalMessageId}\r\n` +
    ...
    async bulkSaveAttachments(args) {
        const userId = args[USER_ID_ARG];
        ...
        try {
            const results = await Promise.all(attachments.map(async (attachmentInfo) => {
